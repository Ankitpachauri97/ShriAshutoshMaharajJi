...
Start_value = int(START[-7:-2]) 
End_value = int(END[-7:-2]) 
...
n = 1
while n < Factor:
    StrikePriceList.append(int(Start_value)+200*n)
    n += 1
print(StrikePriceList)

# -------------------------------TradingSymbolList for all the strike Prices-----------------------------------------------------------------------------------

TradingSymbolList = ['BankNifty', 'INDIA VIX']
...
InstrumentTokenList = [11176962, 264969]
...
def on_ticks(ws, ticks):
    ...
    Nifty_50_LTP = int(InstrumentToken_LTPList[11176962])
    ...
    # ----------------------------Moving up the puts -------------------------------------------------------------------------------------------------------------
        ...
        Closest_instrument_value = min(InstrumentToken_LTPList_PE_check, key=lambda y: abs(float(
            InstrumentToken_LTPList_PE_check[y])-InstrumentToken_LTPList[instrument_second_position]))4
