...
try:
    ...
    for n in TradingSymbolList:
        for k in instrument_token_list1:
            if n == k[2] and '4/21/2022' == str(k[5]):
    ...
    print("no Positions were taken today Closing the code for next date")
    Day = datetime.now().strftime("%A")
    currdate = datetime.now().date() + timedelta(days=int(Weekdays_weekends[Day]))
