...
import math
import pdb
from DeltaCalucation import bns
...
Start_value = int(START[-7:-2]) - int(100)
End_value = int(END[-7:-2]) + int(100)

# -------------------------------StrikePriceList to develop TradingSymbolList ---------------------------------------------------------------------------------

StrikePriceList = []
StrikePriceList.append(int(Start_value))
Factor = (int(End_value)-int(Start_value))/100
n = 1
while n < Factor:
    StrikePriceList.append(int(Start_value)+100*n)
    n += 1
print(StrikePriceList)

# -------------------------------TradingSymbolList for all the strike Prices-----------------------------------------------------------------------------------

TradingSymbolList = ['NIFTY 50', 'INDIA VIX']
for n in StrikePriceList:
    CE_Values = END[0:-7]+str(n)+END[-2:]
    PE_Values = START[0:-7]+str(n)+START[-2:]
...
InstrumentTokenList = [260105, 264969]

data = pd.read_csv("Instrument_value_Bank.csv")
...
# InstrumentToken_strikePrice_dict = {}
# for key, value in InstrumentTokenList_TradingSymbolList_dict:
#     strikePrice = InstrumentTokenList_TradingSymbolList_dict[n][-7:-2]
#     InstrumentToken_LTPList_CE_check.update({key: value[-7:-2]})
...
def RangeCalculator(Nifty, Vix, Days):
    Pct = Vix/math.sqrt(Days)
    LowerLimit = Nifty-Pct*Nifty/100
    UpperLimit = Nifty+Pct*Nifty/100
    print("LowerRange", round(LowerLimit,0))
    print("upperRange", round(UpperLimit,0))
    print(round(UpperLimit,0)-round(LowerLimit,0))
...
def on_ticks(ws, ticks):
    ...
    Nifty_50_LTP = int(InstrumentToken_LTPList[260105])
    ...
    CallOptionDeltas = []
    PutOptionDeltas = []
    CallOptionDeltas = dict.fromkeys(InstrumentToken_LTPList_CE_check, "-1")
    PutOptionDeltas = dict.fromkeys(InstrumentToken_LTPList_PE_check, "-1")
    for keys in InstrumentToken_LTPList_CE_check:
        StrikePrice=InstrumentTokenList_TradingSymbolList_dict[keys][-7:-2]
        bns(StrikePrice,India_Vix_LTP,StrikePrice)
        CallOptionDeltas.update({InstrumentToken_LTPList_CE_check[keys]: call_delta})
        PutOptionDeltas.update({InstrumentToken_LTPList_PE_check[keys]: put_delta})
    pprint(CallOptionDeltas,PutOptionDeltas)
